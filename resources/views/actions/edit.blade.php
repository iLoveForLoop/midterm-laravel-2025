    ...
    <div class="flex flex-col w-full px-10 py-4 uppercase font-semibold text-green-500">
        ...
        <form action="{{ route('music-update', $music->id) }}" method="POST">
            ...
            <div class="w-full flex justify-between items-center mb-10">
                <h1 class="text-4xl ">Edit Music</h1>
            ...
            <div class="flex flex-col gap-1.5 mb-5">
                ...
                <input value="{{ $music->title }}"
                    class="border-b-1 border-green-500 outline-none text-white caret-green-500 " type="text"
                    name="title">
            </div>

            <div class="flex flex-col gap-1.5 mb-5">
                <label class="text-xl" for="artist">Music artist</label>
                <input value="{{ $music->artist }}"
                    class="border-b-1 border-green-500 outline-none text-white caret-green-500 " type="text"
                    name="artist">
            </div>
            ...
            <div class="flex flex-col gap-1.5 mb-5">
                ...
                <select class="border-b-1 border-green-500 outline-none " name="genre">
                    ...
                    <option value="Rock" {{ old('genre', $music->genre) == 'Rock' ? 'selected' : '' }}>Rock</option>
                    <option value="Pop" {{ old('genre', $music->genre) == 'Pop' ? 'selected' : '' }}>Pop</option>
                    <option value="Jazz" {{ old('genre', $music->genre) == 'Jazz' ? 'selected' : '' }}>Jazz</option>
                    <option value="Hip Hop" {{ old('genre', $music->genre) == 'Hip Hop' ? 'selected' : '' }}>Hip Hop
                    </option>
                    <option value="Classical" {{ old('genre', $music->genre) == 'Classical' ? 'selected' : '' }}>Classical
                    </option>
                    <option value="Electronic" {{ old('genre', $music->genre) == 'Electronic' ? 'selected' : '' }}>
                        Electronic</option>
                </select>
            </div>
            ...
            <div class="flex flex-col gap-1.5 mb-5">
                ...
                <input value="{{ $music->release_date }}" class="border-b-1 border-green-500 outline-none text-green-500 "
